...
def test_to_dict_exposes_stable_english_contract_keys():
    module = PhenotypingModule({"phenotyping": {}})
    metrics = PhenotypeMetrics(
        azolla_area_pixels=100.0,
        azolla_area_m2=0.01,
        ...
        saci_index=0.2,
        chlorophyll_index=1.5,
        stress_browning_percent=3.0,
        stress_yellowing_percent=4.0,
        stress_score=7.0,
        density_low_percent=10.0,
        density_medium_percent=20.0,
        density_high_percent=70.0,
    ...
    payload = module.to_dict(metrics, language="en", schema_version="1.0.0")
    ...
    for key in (
        ...
        "browning_percent",
        "yellowing_percent",
        "growth_rate_percent_day",
        "health_score",
        "stress_score",
    ):
        assert key in payload
    assert payload["color_indices"] == {
        "agi": 0.1,
        "saci": 0.2,
        "chlorophyll_index": 1.5,
    }
    assert payload["health_score"] == 93.0
