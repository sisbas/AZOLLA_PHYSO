...
if grep -nE '^(<<<<<<<|=======|>>>>>>>)' README.md >/dev/null; then
  echo "ERROR: README.md contains unresolved merge conflict markers."
  grep -nE '^(<<<<<<<|=======|>>>>>>>)' README.md || true
  exit 1
fi

if ! awk 'NR==1{ok=($0=="---")} NR>1&&NR<40&&$0=="---"{end=1} END{exit !(ok&&end)}' README.md; then
  echo "ERROR: README.md must start with a valid YAML front matter block delimited by ---."
  exit 1
fi
